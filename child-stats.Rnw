...
for(c in unique(d$school.id)) {
  OLS <- lm(y ~ treated + wave + treated*wave, data = d, subset = school.id == c)
}
coef(OLS)
...
kipols <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP)
...
kipols1 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==1)


# For school2

kipols2 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==2)


#For school3

kipols3 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==3)


#For school4

kipols4 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==4)


#For school5

kipols5 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==5)


#For school6

kipols6 <- lm(v07a ~ welle + pupil + v01a + Treatment, KIP, subset = school==6)



# cat(paste('\\newcommand{\\allschools}{\n', sep=''))
# stargazer(sapply(list(all = kipols, school1 = kipols1, school2 = kipols2, school3 = kipols3, school4 = kipols4, school5 = kipols5, school6 = kipols6),
#        function(x) coef(summary(x))["Treatment", c("Estimate", "Std. Error")]), )
# cat(paste('}\n'))

# sp <- ggplot(data = KIP, aes(x=welle, y=v07a), subset(Treatement==1))


# sp + geom_point()
# sp <- sp + geom_point()

# sp+ ggtitle("my first scatterplot") + stat_smooth(lm)
